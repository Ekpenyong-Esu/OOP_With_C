...
void HistogramDisplay_Init(HistogramDisplay* const me) {
    me->itsTMDQueue = NULL;
    {
        ...
        TMDQueue_subscribe(me->itsTMDQueue, (UpdateFuncPtr) HistogramDisplay_updateHistogram);
    }
}

void HistogramDisplay_Cleanup(HistogramDisplay* const me) {
    /* remove yourself from server subscription list */
    TMDQueue_unsubscribe(me->itsTMDQueue, (UpdateFuncPtr) HistogramDisplay_updateHistogram);
    cleanUpRelations(me);
}
