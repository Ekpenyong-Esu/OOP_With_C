...
ASSERT_EQ(BASIC, BASIC); // Assert that the task type is BASIC
ASSERT_STREQ(basicTask->title, "Basic Task"); // Assert that the task name is correct
...
Task* urgentTask = createTaskWithType(BASIC,2, "Urgent Task", "This is an urgent task", 1);
ASSERT_NE(urgentTask, nullptr); // Assert that the task is not null
ASSERT_EQ(URGENT, URGENT); // Assert that the task type is URGENT
ASSERT_STREQ(urgentTask->title, "Urgent Task"); // Assert that the task name is correct
...
EXPECT_THAT(output, testing::ContainsSubstring("Basic Task"));
EXPECT_THAT(output, testing::ContainsSubstring("This is a basic task"));
EXPECT_THAT(output, testing::ContainsSubstring("Priority: 2"));
